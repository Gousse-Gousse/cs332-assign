...
abstract class TweetSet {
  ...
  def mostRetweeted: Tweet = {
    var retTweet: Tweet = null
    this.foreach(
      tweet =>
        if (retTweet==null) retTweet = tweet
        else if (retTweet.retweets < tweet.retweets) retTweet = tweet
    )
	  retTweet
  }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def descendingByRetweet : TweetList = {
    lazy val tweetMostRetweet = this.mostRetweeted
	  new Cons(tweetMostRetweet,this.remove(tweetMostRetweet).descendingByRetweet)
  } 
