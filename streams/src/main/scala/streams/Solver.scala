...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
	  ...
	  initial match {
		  case (b,h) #:: tail =>
			  (b, h) #:: from(
				    tail #::: newNeighborsOnly( neighborsWithHistory(b,h), explored ),
				    explored+b
			    )
  ...
  lazy val pathsToGoal: Stream[(Block, List[Move])] =
	  pathsFromStart.filter{case (b, history) => done(b)}
  ...
  lazy val solution: List[Move] = pathsToGoal match {
	  case (b, history) #:: tail=> history.reverse
	  case Stream.Empty => List.empty
  }
	
	def check_solution: Boolean = {
		done(solution.foldLeft(startBlock)((b, move) => if (b.isLegal) b.moveBlock(move) else b))
	}
}
